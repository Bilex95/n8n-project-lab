...
| [czlonkowski/n8n-mcp](https://github.com/czlonkowski/n8n-mcp) | MIT | Extremely High | `mcp/` | Build→validate→test→regression methodology; tool catalog reference; security principles (read-only where possible, never modify prod directly) | The application itself — used as an MCP server if/when installed, not vendored into the lab |
| [czlonkowski/n8n-skills](https://github.com/czlonkowski/n8n-skills) | MIT (hooks layer adapts Apache-2.0 patterns from n8n-io/skills, see their NOTICES) | Extremely High | `mcp/` | The 14-skill catalog as a checklist of concepts this lab's own QA should cover (expressions, error handling, sub-workflows, AI agents, etc.) | Not yet installed — requires n8n-mcp configured first (its own stated prerequisite) |
...
| [lemony-ai/cascadeflow](https://github.com/lemony-ai/cascadeflow) *(unconfirmed match — see note below)* | MIT | High (supporting) | `ai-agents/` | Model-cascading/routing QA ideas: test the *routing decision*, not just final output; budget/tool-call limits | Full framework |
...
**Note on cascadeflow:** no exact URL was given; GitHub search matched `lemony-ai/cascadeflow` ("Cascading runtime for AI agents — optimize cost, latency, quality, and policy decisions inside the agent loop"), which matches the described concepts closely. Flagged for confirmation before being treated as settled.
